...
import com.example.calorietracker.databinding.ActivityFormTambahMakananBinding

class FormTambahMakananActivity : AppCompatActivity() {

    private lateinit var binding: ActivityFormTambahMakananBinding
    private lateinit var makanan: Makanan
    private lateinit var makananViewModel: MakananViewModel

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityFormTambahMakananBinding.inflate(layoutInflater)
        setContentView(binding.root)

        makanan = intent.getSerializableExtra("makanan") as Makanan

        binding.tvNamaMakanan.setText(makanan.namaMakanan)
        binding.tvJumlahKalori.setText(makanan.jumlahKalori)
    ...
    private fun tambahMakanan() {
        ...
        val namaMakanan: String = binding.tvNamaMakanan.text.toString()
        val takaranSaji: Int = binding.edtTakaranSaji.text.toString().toInt()
        val jumlahKalori: Int = binding.tvJumlahKalori.text.toString().toInt()*takaranSaji/100
        ...
        // Save ke Room Database dengan ViewModel
        makananViewModel.tambahMakanan(makanan)

        // Menampilkan Toast kemudian finish
